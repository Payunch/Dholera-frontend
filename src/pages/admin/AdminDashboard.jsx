...
import { AdminRegistrationReview } from '../../components/AdminRegistrationReview';
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Typography variant="h4" sx={{ fontWeight: 800 }}>Admin Master Control</Typography>
        <Tabs 
          value={activeTab} 
          onChange={(_, v) => setActiveTab(val => v)}
          sx={{ 
            bgcolor: 'grey.100', 
            borderRadius: 2, 
            p: 0.5,
            '& .MuiTabs-indicator': { display: 'none' },
            '& .MuiTab-root': { borderRadius: 1.5, minHeight: 40, px: 3, fontWeight: 700 },
            '& .Mui-selected': { bgcolor: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', color: 'primary.main' }
          }}
        >
          <Tab icon={<AssignmentIcon sx={{ mr: 1, fontSize: 18 }} />} iconPosition="start" label="Leads" />
          <Tab icon={<GroupsIcon sx={{ mr: 1, fontSize: 18 }} />} iconPosition="start" label="Professionals" />
          <Tab icon={<AnalyticsIcon sx={{ mr: 1, fontSize: 18 }} />} iconPosition="start" label="Insights" />
        </Tabs>
      </Box>
