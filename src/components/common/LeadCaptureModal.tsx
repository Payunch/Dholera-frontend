...
export default function LeadCaptureModal() {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-500">
      <div className="bg-white dark:bg-[#0B132B] w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-slate-800 animate-in zoom-in-95 duration-500">
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
              ...
              <button 
                ...
                className="w-full h-16 mt-4 rounded-2xl bg-[#FF7A00] text-white text-xs font-black uppercase tracking-[0.2em] hover:bg-orange-600 disabled:bg-slate-800 disabled:text-slate-600 transition-all shadow-xl shadow-orange-600/20 flex items-center justify-center gap-3 active:scale-95"
              >
                {status === 'loading' ? (
